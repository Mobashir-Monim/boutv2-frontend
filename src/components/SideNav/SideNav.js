...
const SideNav = ({ showNav, toggleNav, collapseNav }) => {
    ...
    return <div className={`${widthClasses} flex flex-col py-2 lg:py-10 gap-1 lg:px-5 z-[50] fixed overflow-hidden bg-blue-700 dark:bg-blue-800 transition-all duration-300 ease-linear drop-shadow-lg`}>
        <div className={`flex flex-row justify-start transition-all duration-300 ease-linear ml-6 mt-2 lg:ml-1`}>
            <span className={`bg-white rounded-full cursor-pointer relative p-2 inline text-center material-icons-round font-bold transition-all duration-500 ease-linear ${arrowContainerClasses}`} onClick={toggleNav}>
                <span className={`material-icons-round font-bold transition-all duration-300 ease-linear text-black ${arrowClasses}`} onClick={toggleNav}>arrow_forward_ios</span>
            </span>
        </div>
